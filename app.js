 ...
 * Versão: 5.2 - Atualização da URL do Apps Script
 */
...
class CollageBuilder {
    async build(session) {
        ...
        // Removido crossOrigin="anonymous" que bloqueava imagens locais Base64 no celular
        ...
        // UPGRADE DE RESOLUÇÃO (50% Maior): Célula de 320 para 480 px
        ...
        // Ajuste no tamanho da fonte para a nova resolução
        ...
        // Qualidade aumentada de 0.90 para 0.95
...
document.getElementById('inputPhoto').onchange = e => { Array.from(e.target.files).forEach(f => { const r = new FileReader(); r.onload = ev => app.session.addPhoto(ev.target.result); r.readAsDataURL(f); }); };