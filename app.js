 ...
 * Versão: 5.0 - Auditoria, Finanças e Divergência
 */
...
class Session {
    ...
    registerPackage(rawCode) {
        ...
        if (code.includes('hash_code')) {
            type = 'ml'; try { displayCode = JSON.parse(code).hash_code || "ML"; } catch (e) {}
        } else if (/^BR[A-Z0-9]{10,16}$/i.test(code)) { type = 'shopee'; }
    ...
    clearAll() { if (confirm("Apagar leituras?")) { this.counts = { ml: 0, shopee: 0, avulso: 0, total: 0 }; this.codesMap.clear(); this.updateUI(); } }
...
class CollageBuilder {
    async build(session) {
        ...
        ctx.fillStyle = '#0d0f12'; ctx.fillRect(0, 0, W, H); ctx.fillStyle = '#161a1f'; ctx.fillRect(0, 0, W, HDR); ctx.fillStyle = '#f97316'; ctx.fillRect(0, HDR - 3, W, 3);
        ...
        const d = new Date(); ctx.fillStyle = '#6b7280'; ctx.font = '12px monospace';
...
class ExportController {
    ...
    async toWhatsApp() {
        ...
        if (expectedStr === "" || isNaN(expected) || expected <= 0) {
            ...
            return alert("Obrigatório: Preencha a Qtd. Esperada (Manual) na primeira aba antes de finalizar.");
        }

        if (scanned !== expected) {
            document.getElementById('divergenceText').innerHTML = `Gaiola: <b>${expected}</b> | Lidos: <b>${scanned}</b>`;
    ...
    async confirmAndSend() {
        ...
        try {
            ...
            const payload = {
                ...
                codes: Array.from(s.codesMap.keys()).join(" | "),
                image: b64
            };
...
document.getElementById('inputSelfie').onchange = e => {
    ...
    r.onload = ev => { app.session.selfie = ev.target.result; document.getElementById('previewSelfie').src = ev.target.result; document.getElementById('previewSelfie').style.display = 'block'; document.getElementById('selfiePlaceholder').style.display = 'none'; app.session.saveBackup(); };
