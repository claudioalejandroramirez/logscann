...
class ExportController {
    constructor(parent) {
        this.parent = parent;
        // IMPORTANTE: COLOQUE A URL DO SEU GOOGLE APPS SCRIPT AQUI ABAIXO
    ...
    async toSheets() {
        ...
        if(this.sheetsUrl === "SUA_URL_DO_GOOGLE_APPS_SCRIPT_AQUI") return alert("Configure a URL do Sheets no código fonte!");
        ...
        if (!navigator.onLine) {
            ...
            alert("⚠️ Sem internet! Salvo para envio automático.");
            this.parent.session.reset(); 
            btn.disabled = false; btn.textContent = "📊 ENVIAR PARA PLANILHA";
            return;
        }
        ...
        try {
            ...
            this.saveToQueue(payload);
            alert("⚠️ Instabilidade na rede! Salvo na fila interna.");
            this.parent.session.reset();
        } finally {
            btn.disabled = false; btn.textContent = "📊 ENVIAR PARA PLANILHA";
        }
    ...
    async syncQueue() {
        ...
        const btnSync = document.getElementById('btnSync'); btnSync.textContent = "⏳ SINCRONIZANDO...";
    ...
    updateSyncUI() {
        ...
        if (queue.length > 0) { btnSync.classList.remove('hidden'); btnSync.textContent = `🔄 SINCRONIZAR PENDENTES (${queue.length})`; } 
