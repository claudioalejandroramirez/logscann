 ...
 * Versão: 5.3 - Extração limpa de ID (JSON Mercado Livre)
 */
...
class Session {
    ...
    registerPackage(rawCode) {
        ...
        // NOVO CÓDIGO DE EXTRAÇÃO: Lê o JSON e extrai apenas o ID do Mercado Livre
        if (code.startsWith('{') || code.includes('hash_code')) {
            type = 'ml'; 
            try { 
                const parsed = JSON.parse(code); 
                // Extrai o 'id', se não houver extrai o 'hash_code'
    ...
    async confirmAndSend() {
        ...
        try {
            ...
            const payload = {
                ...
                // NOVO CÓDIGO DE EXPORTAÇÃO: Envia apenas os IDs limpos (ex: 2000011598082527), e não o JSON cru
