...
class CollageBuilder {
  async build(session) {
    ...
    ctx.fillText('SAÍDA FLEX VELOZZ', PAD + 10, 40);
    ctx.fillStyle = '#e8eaf0';
    ctx.font = '22px monospace';
    ctx.fillText(`${session.driver} | Op: ${session.operator}`, PAD + 10, 70);
    const d = new Date();
    ctx.fillStyle = '#6b7280';
    ctx.font = '18px monospace';
    ctx.fillText(
      `${d.toLocaleDateString('pt-BR')} ${d.toLocaleTimeString('pt-BR')} | Total: ${session.counts.total}`,
      PAD + 10,
      100
    );
